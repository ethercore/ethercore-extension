...
class DetectTokensController {
  ...
  async detectNewTokens () {
    ...
    ethContract.balances([this.selectedAddress], tokensToDetect, (error, result) => {
      if (error) {
        warn(`EtherCore Extension - DetectTokensController single call balance fetch failed`, error)
        return
      }
  ...
  async detectTokenBalance (contractAddress) {
    ...
    ethContract.balanceOf(this.selectedAddress, (error, result) => {
      if (!error) {
        ...
        warn(`EtherCore Extension - DetectTokensController balance fetch failed for ${contractAddress}.`, error)
      }
    })
  }
