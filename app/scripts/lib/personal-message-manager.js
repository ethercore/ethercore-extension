...
module.exports = class PersonalMessageManager extends EventEmitter {
  ...
  addUnapprovedMessageAsync (msgParams, req) {
    return new Promise((resolve, reject) => {
      if (!msgParams.from) {
        reject(new Error('EtherCore Message Signature: from field is required.'))
      }
      ...
      this.once(`${msgId}:finished`, (data) => {
        switch (data.status) {
          ...
          case 'rejected':
            return reject(new Error('EtherCore Message Signature: User denied message signature.'))
          default:
            return reject(new Error(`EtherCore Message Signature: Unknown problem: ${JSON.stringify(msgParams)}`))
        }
      })
    })
  }
